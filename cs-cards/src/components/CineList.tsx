      ...
      <div className="mb-8 flex items-center justify-between">
        <div className="flex items-center gap-3 md:gap-4">
          <button
            ...
            className="flex h-9 w-9 md:h-10 md:w-10 items-center justify-center rounded-full border border-[#112332] bg-[#071118] text-slate-300 transition-all hover:bg-[#0a1824] hover:text-[#4df2d6] hover:border-teal-500/40 cursor-pointer shrink-0 shadow-sm"
          >
      ...
      <AnimatePresence mode="popLayout">
        {savedMovies.length === 0 ? (
          ...
          >
            ...
            <div className="relative mb-6 flex h-24 w-24 items-center justify-center rounded-full bg-[#071118] border border-[#112332] shadow-[0_0_30px_rgba(77,242,214,0.15)] text-slate-500">
              <Film size={40} className="stroke-[1.5] text-slate-400" />
              <div className="absolute -right-1 -top-1 flex h-6 w-6 items-center justify-center rounded-full bg-teal-500/20 text-[#4df2d6] border border-teal-500/40">
            ...
            <motion.button
              onClick={onBrowseMovies}
              className="rounded-full bg-[#75D4CB] px-8 py-3 text-sm font-bold text-[#001017] shadow-lg shadow-teal-500/20 hover:bg-[#22A498] transition-all cursor-pointer"
          ...
          >
            {savedMovies.map((movie) => (
              <motion.div
                ...
                className="group relative flex items-center justify-between gap-4 rounded-2xl border border-[#112332] bg-[#071118]/80 backdrop-blur-md p-3 md:p-4 hover:border-teal-500/50 transition-all hover:bg-[#0a1824] cursor-pointer overflow-hidden shadow-xl"
                onClick={() => onSelectMovie(movie)}
              >
                {/* Subtle soft-glow light-leak inside hovered item */}
                <div className="absolute -left-12 -top-12 h-24 w-24 rounded-full bg-[#4df2d6]/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-2xl pointer-events-none" />

                {/* Left side: Thumbnail Poster & Text details */}
                <div className="flex items-center gap-4 flex-1 min-w-0">
                  {/* Poster Thumbnail */}
                  <div className="relative h-16 w-12 md:h-20 md:w-16 flex-shrink-0 overflow-hidden rounded-lg border border-[#112332] bg-slate-900 shadow-inner">
                    <img
                      src={movie.posterUrl}
                      alt={movie.title}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                      {/* IMDb rating display */}
                      <div className="flex items-center gap-1 bg-teal-950/40 border border-teal-800/30 rounded px-1.5 py-0.5 text-[11px] font-mono font-medium text-[#4df2d6]">
